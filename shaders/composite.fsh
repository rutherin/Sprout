...
float getShadows(vec3 viewSpace, int index, const int ditherSize, float lightmap) {
	...
	if ((matIDs >= 3.5 &&  matIDs < 4.5)) size = 1.0 * (1.0 * ((shadowMapResolution / (shadowMapResolution * 1.1))));
	#endif
...
if (depth0 >= 1.0) {
     ...
     color += VL().x * hgPhase(dot(lightvec, viewvec), 0.34) * VL_Strength * ((SunColor * 1.7 * watermultiplier) + (MoonColor * 26)) * 0.2 * multiplier * colormult2 * 0.15 * (0.2 / (sunAngle + 0.05));
     #endif
}
