const vec2 celshadeoffset[4] = vec2[4](vec2(1.0,0.0),vec2(1.0,1.0),vec2(0.0,1.0),vec2(-1.0,1.0));
...
float celshade(sampler2D depth, float forcefull){
	...
	for (int i = 0; i < 4; i++){
		...
		outline *= mix(clamp(1.0-(z-(sampleza+samplezb))*0.5,0.0,1.0),1.0,min(min(sampleza,samplezb)/dist,0.8+0.2*isEyeInWater)*(1.0-forcefull));
	}
