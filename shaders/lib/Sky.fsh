...
vec3 GetSunColorZom() {
	return sky_atmosphereTransmittance(vec3(0.0, atmosphere_planetRadius, 0.0), normalize(mat3(gbufferModelViewInverse) * (sunPosition * 0.01)), 3) * vec3(1.0, 0.96, 0.95);
}
...
vec3 sky_atmosphereA(vec3 background, vec3 viewVector, vec3 upVector, vec3 sunVector, vec3 moonVector, vec3 sunIlluminance, vec3 moonIlluminance, const int iSteps, inout vec3 transmittance, vec3 ambientCol2) {
	...
	vec3 SunColor = pow(GetSunColorZom(), vec3(2.0)) * vec3(2.2, 1.28, 1.0) * 3.9;
	...
	return vec3(0.1) * ambientCol2 + vec3(0.0, 0.07, 0.3);
}
